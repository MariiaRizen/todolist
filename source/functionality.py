...
def search_task_by_title(query: str):
    ...
    return [x for x in todo_list if query in x['title']]
...
def show_title_time_off():
    ...
    time_off_list = [x for x in todo_list if x['due_date'] < datetime.now()]
    print(time_off_list)
...
def save_as_csv_file():
    ...
    with open('data/todolistcsvfile.csv', 'w', newline='') as f:
        ...
        w.writerow([
            ...
            "due_date",
            "done"])
        for my_dict in todo_list:
            w.writerow(my_dict.values())



def downlad_test_data():
    """ Download test data"""
    # global todo_list
    todo_list = TEST_DATA
    print(todo_list)
